import '../styles/Footer.css'
import skillsyncLogo from '../assets/BGLogo.jpg'

function Footer({ onNavigate = () => {} }) {
  const currentYear = new Date().getFullYear()

  return (
    <footer className="footer">
      <div className="footer-content">
        {/* Company Section */}
        <div className="footer-section footer-brand">
          <div className="footer-logo">
            <img src={skillsyncLogo} alt="SkillSync Logo" className="footer-logo-img" />
            <span className="footer-logo-text">SkillSync</span>
          </div>
          <p className="footer-tagline">
            Connect. Learn. Exchange.
          </p>
          <p className="footer-description">
            Transforming how the world learns and shares skills through peer-to-peer connections.
          </p>
        </div>

        {/* Quick Links */}
        <div className="footer-section">
          <h3 className="footer-heading">Quick Links</h3>
          <ul className="footer-links">
            <li>
              <button onClick={() => onNavigate('home')} className="footer-link">
                Home
              </button>
            </li>
            <li>
              <button onClick={() => onNavigate('about')} className="footer-link">
                About
              </button>
            </li>
            <li>
              <button onClick={() => onNavigate('vision')} className="footer-link">
                Vision
              </button>
            </li>
            <li>
              <button onClick={() => onNavigate('team')} className="footer-link">
                Team
              </button>
            </li>
            <li>
              <button onClick={() => onNavigate('contact')} className="footer-link">
                Contact
              </button>
            </li>
            <li>
              <button onClick={() => onNavigate('faqs')} className="footer-link">
                FAQs
              </button>
            </li>
          </ul>
        </div>

        {/* Connect Section */}
        <div className="footer-section">
          <h3 className="footer-heading">Connect With Us</h3>
          <div className="footer-social">
            <a
              href="https://www.instagram.com/skillsync.bh?igsh=MXdmZTdqbzZjbG5wbA=="
              target="_blank"
              rel="noopener noreferrer"
              className="footer-social-link"
              aria-label="Instagram"
            >
              <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
                <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/>
              </svg>
              <span>Instagram</span>
            </a>
            <a
              href="https://www.tiktok.com/@skillsync.bh?_r=1&_t=ZS-94HVcmQ6jWa"
              target="_blank"
              rel="noopener noreferrer"
              className="footer-social-link"
              aria-label="TikTok"
            >
              <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
                <path d="M19.59 6.69a4.83 4.83 0 0 1-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 0 1-5.2 1.74 2.89 2.89 0 0 1 2.31-4.64 2.93 2.93 0 0 1 .88.13V9.4a6.84 6.84 0 0 0-1-.05A6.33 6.33 0 0 0 5 20.1a6.34 6.34 0 0 0 10.86-4.43v-7a8.16 8.16 0 0 0 4.77 1.52v-3.4a4.85 4.85 0 0 1-1-.1z"/>
              </svg>
              <span>TikTok</span>
            </a>
            <a
              href="mailto:skillsync.company@gmail.com"
              className="footer-social-link"
              aria-label="Email"
            >
              <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
                <path d="M20 4H4c-1.1 0-1.99.9-1.99 2L2 18c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm0 4l-8 5-8-5V6l8 5 8-5v2z"/>
              </svg>
              <span>Email</span>
            </a>
          </div>
          <div className="footer-email">
            <p className="footer-email-label">Email us:</p>
            <a href="mailto:skillsync.company@gmail.com" className="footer-email-link">
              skillsync.company@gmail.com
            </a>
          </div>
        </div>
      </div>

      {/* Bottom Bar */}
      <div className="footer-bottom">
        <div className="footer-bottom-content">
          <p className="footer-copyright">
            © {currentYear} SkillSync. All rights reserved.
          </p>
          <div className="footer-legal">
            <button 
              className="footer-legal-link"
              onClick={() => onNavigate('privacy')}
            >
              Privacy Policy
            </button>
            <span className="footer-divider">•</span>
            <button 
              className="footer-legal-link"
              onClick={() => onNavigate('terms')}
            >
              Terms of Service
            </button>
          </div>
        </div>
      </div>
    </footer>
  )
}

export default Footer
